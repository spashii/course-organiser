...
static void __print(avltree_t* me, int idx, int d) {
    ...
    if (me->size <= idx || !me->nodes[idx]) {
        printf("\n");
        return;
    }

    printf("%ld\n", (long)me->nodes[idx]->id);
...
void avltree_print2(avltree_t* me) {
    ...
    for (i = 0; i < me->size; i++)
        printf("%ld%c", (long)me->nodes[i]->id, i == me->size ? '|' : ' ');
    printf("\n");
}
...
void avltree_rotate_right(avltree_t* me, int idx) {
    ...
    me->nodes[__child_r(__child_l(idx))] = NULL;
...
void avltree_insert(avltree_t* me, void* c) {
    int i;
    node_t* n;
    ...
    for (i = 0; i < me->size;) {
        ...
        long r = me->cmp((*n)->code, ((node_t)c)->code);
